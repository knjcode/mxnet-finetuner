...
for CUR_EPOCH in $EPOCHS; do
  ...
  python3 util/extract_feature.py "$CONFIG_FILE" "$MODEL_IMAGE_SIZE" "$LAYER_BEFORE_FULLC" "test" "$MODEL" "$CUR_EPOCH"

  # save config.yml
  CONFIG_LOG="logs/$MODEL-$(printf "%04d" $CUR_EPOCH)-extract-feature-config.yml"
