...
def get_symbol(num_classes, num_layers, image_shape, num_group=32, conv_workspace=256, dtype='float32', bn_mom=0.9, **kwargs):
    ...
    return resnext(units=units, num_stage=4, filter_list=[64, 256, 512, 1024, 2048] if num_layers >=50
                    else [64, 64, 128, 256, 512], ratio_list=ratio_list, num_class=num_classes, num_group=num_group, data_type="imagenet", drop_out=0.0, bottle_neck = True
                    if num_layers >= 50 else False, bn_mom=bn_mom, workspace=conv_workspace, memonger=False)
